...
class Metrics:
    def __init__(self) -> None:
        ...
        self.deckbuilding_samples = self.deckbuilding_correct = 0
    ...
    def update(
        ...
        if deckbuilding_count:
            ...
            self.deckbuilding_correct += int(correct[deckbuilding].sum().detach().cpu())
    ...
    def as_dict(self) -> dict[str, float]:
        ...
        return {
            ...
            "deckbuilding_accuracy": self.deckbuilding_correct / b,
        }
...
def make_build_examples(
    ...
    pool_rows, deck_rows, pack_nos, pick_nos, build_steps, target_masks = [], [], [], [], [], []
    pool_start = 2 + cards
...
def run_epoch(
    ...
    for step, batch in enumerate(iter_phase_batches(chunks, cards, batch_size, val_fraction, epoch, split, max_batches, train_build), 1):
        ...
        if training and LOG_EVERY > 0 and step % LOG_EVERY == 0:
            ...
            print(f"  step {step:5d} | loss {now['loss']:.4f} | top1 {now['top1']:.4f} | deckbuild {now['deckbuilding_accuracy']:.4f} | nontriv {now['nontrivial_top1']:.4f} | lr {opt.param_groups[0]['lr']:.2e} | {speed:,.0f} rows/s")
...
def format_metrics(prefix: str, metrics: dict[str, float]) -> str:
    return f"{prefix} loss {metrics['loss']:.4f} | top1 {metrics['top1']:.4f} | top3 {metrics['top3']:.4f} | deckbuild {metrics['deckbuilding_accuracy']:.4f} | mrr {metrics['mrr']:.4f} | nontriv {metrics['nontrivial_top1']:.4f} | {metrics['seconds']:.1f}s"
